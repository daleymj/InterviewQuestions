...
employeeData.forEach(employee => {
    ...
    employee.timePunch.forEach(punch => {
        const jobRate = jobMeta[punch.job].rate;
        const benefitsRate = jobMeta[punch.job].benefitsRate;
        const startTime = new Date(punch.start);
        const endTime = new Date(punch.end);
        const hoursWorked = (endTime - startTime) / 3600000;
    ...
    employee.timePunch.forEach(punch => {
        ...
        if (hoursWorked <= regularHours) {
            wageTotal += hoursWorked * jobRate;
            benefitsTotal += hoursWorked * benefitsRate;
            console.log(employee.employee,wageTotal);
        } else if (hoursWorked <= regularHours + overtimeHours) {
            wageTotal += regularHours * jobRate;
            benefitsTotal += regularHours * benefitsRate;
            wageTotal += (hoursWorked - regularHours) * jobRate * 1.5;
            benefitsTotal += (hoursWorked - regularHours) * benefitsRate * 1.5;
        } else {
            wageTotal += regularHours * jobRate;
            benefitsTotal += regularHours * benefitsRate;
            wageTotal += overtimeHours * jobRate * 1.5;
            benefitsTotal += overtimeHours * benefitsRate * 1.5;
            wageTotal += (hoursWorked - regularHours - overtimeHours) * jobRate * 2;
            benefitsTotal += (hoursWorked - regularHours - overtimeHours) * benefitsRate * 2;
        }
    });

    result[employee.employee] = {
        employee: employee.employee,
        regular: regularHours.toFixed(4),
        overtime: overtimeHours.toFixed(4),
