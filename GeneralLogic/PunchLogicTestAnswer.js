...
function calculateHours(start, end) {
    ...
    const delta = (endTime - startTime) / 1000 / 3600; // convert milliseconds to hours
    return delta;
}

// Create job rate and benefits lookup
const jobLookup = {};
jobMeta.forEach(job => {
    jobLookup[job.job] = { rate: job.rate, benefitsRate: job.benefitsRate };
});
...
employeeData.forEach(employee => {
    ...
    const timePunches = employee.timePunch;
    ...
    timePunches.forEach(punch => {
        ...
        const jobRate = jobLookup[job].rate;
        const jobBenefitsRate = jobLookup[job].benefitsRate;
        ...
        wageTotal += regular * jobRate + overtime * jobRate * 1.5 + doubletime * jobRate * 2;
        benefitsTotal += hours * jobBenefitsRate;
    });
